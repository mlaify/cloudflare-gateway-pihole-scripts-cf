...
// In the Cloudflare Workers runtime there is no filesystem and no .env file:
// configuration arrives as `process.env` populated from Wrangler vars/secrets
// (via the nodejs_compat flag). Only load a .env file when running under Node.
const isCloudflareWorker =
  globalThis.navigator?.userAgent === "Cloudflare-Workers";

if (!isCloudflareWorker) dotenv.config();
...
// Number of entries packed into each Zero Trust list.
// Standard accounts allow up to 1,000 entries per list; Enterprise allows 5,000.
// The account is also capped at 100 lists total, so on Enterprise you must use
// 5,000 to reach the 500,000-entry ceiling (100 lists x 5,000) without exceeding
// the list count. Configurable via CLOUDFLARE_LIST_ITEM_SIZE.
