...
// The list-items endpoint does not document a max per_page, and larger lists
// (e.g. 5,000-entry Enterprise lists) exceed whatever the server-side cap is.
// Requesting more than the server allows silently truncates the response, which
// would make the sync diff repeatedly re-add the missing tail. 1,000 is a safe
// page size that is honoured across account tiers.
const LIST_ITEMS_PAGE_SIZE = Math.min(LIST_ITEM_SIZE, 1000);

/**
 * Gets all Zero Trust list items, following pagination.
 *
 * API docs: https://developers.cloudflare.com/api/operations/zero-trust-lists-zero-trust-list-items
 * @param {string} id The id of the list.
 * @returns {Promise<{result: Object[], result_info: Object}>}
 */
const getZeroTrustListItems = async (id) => {
  const items = [];
  let page = 1;
  let totalCount = 0;

  // Loop pages until we've collected every item. We stop when a page returns
  // fewer items than requested (the last page) or once we've gathered total_count.
  // eslint-disable-next-line no-constant-condition
  while (true) {
    const { result, result_info } = await requestGateway(
      `/lists/${id}/items?per_page=${LIST_ITEMS_PAGE_SIZE}&page=${page}`,
      { method: "GET" }
    );

    if (result_info?.total_count != null) totalCount = result_info.total_count;

    const batch = result || [];
    items.push(...batch);

    if (batch.length < LIST_ITEMS_PAGE_SIZE) break;
    if (totalCount && items.length >= totalCount) break;

    page++;
  }

  return { result: items, result_info: { total_count: totalCount || items.length } };
};
...
export const synchronizeZeroTrustLists = async (items) => {
  ...
  for (const list of cgpsLists) {
    ...
    if (result_info.total_count > LIST_ITEM_SIZE) {
      console.log(`List ${list.name} contains ${result_info.total_count} entries, more than the configured LIST_ITEM_SIZE of ${LIST_ITEM_SIZE}. It was likely created with a larger size; consider deleting and recreating it so capacity accounting stays consistent.`);
    }
...
/**
 * Creates or updates a general Gateway policy (allow OR block, any layer, with
 * explicit precedence). Used by cf_policies_create.js. Idempotent by name.
 * @param {Object} policy
 * @param {string} policy.name Rule name (must be unique).
 * @param {"allow"|"block"|"off"} policy.action
 * @param {string[]} policy.filters e.g. ["dns"], ["http"], ["l4"].
 * @param {string} policy.traffic Wirefilter expression.
 * @param {number} [policy.precedence] Lower = evaluated earlier.
 * @param {boolean} [policy.enabled] Defaults to true.
 * @param {string} [policy.description]
 * @param {Object} [policy.ruleSettings]
 * @param {Object[]} [existingRules] Pre-fetched rules, to avoid refetching.
 * @returns {Promise<"created"|"updated">}
 */
export const upsertGatewayPolicy = async (policy, existingRules) => {
  const rules = existingRules || (await getZeroTrustRules()).result || [];
  const existing = rules.find((r) => r.name === policy.name);

  const body = JSON.stringify({
    name: policy.name,
    description:
      policy.description ||
      "Managed by CGPS (policies.config.js). Changing the name breaks reconciliation.",
    enabled: policy.enabled !== false,
    action: policy.action,
    filters: policy.filters,
    traffic: policy.traffic,
    ...(policy.precedence != null ? { precedence: policy.precedence } : {}),
    rule_settings: policy.ruleSettings || {},
  });

  if (existing) {
    await requestGateway(`/rules/${existing.id}`, { method: "PUT", body });
    return "updated";
  }
  await requestGateway("/rules", { method: "POST", body });
  return "created";
};
